image_angle=0
ilosc_kolecorerow=5
i=0

bulpon[i]=instance_create(x-200,y-400,o_kolecorer)
bulpon[i].spadnie=0
bulpon[i].image_xscale=1.3
bulpon[i].image_yscale=1.3
bulpon[i].image_angle=0
bulpon_nagraj_x[i]=x-200
bulpon_nagraj_y[i]=y-400

i+=1
bulpon[i]=instance_create(x-300,y-80,o_kolecorer)
bulpon[i].spadnie=0
bulpon[i].image_xscale=1.3
bulpon[i].image_yscale=1.3
bulpon[i].image_angle=0
bulpon_nagraj_x[i]=x-300
bulpon_nagraj_y[i]=y-80


i+=1
bulpon[i]=instance_create(x+400,y-300,o_kolecorer)
bulpon[i].spadnie=0
bulpon[i].image_xscale=1.3
bulpon[i].image_yscale=1.3
bulpon[i].image_angle=0
bulpon_nagraj_x[i]=x+400
bulpon_nagraj_y[i]=y-300


i+=1
bulpon[i]=instance_create(x+400,y-150,o_kolecorer)
bulpon[i].spadnie=0
bulpon[i].image_xscale=1
bulpon[i].image_yscale=1
bulpon[i].image_angle=20
bulpon_nagraj_x[i]=x+400
bulpon_nagraj_y[i]=y-150

i+=1
bulpon[i]=instance_create(x+400,y+50,o_kolecorer)
bulpon[i].spadnie=0
bulpon[i].image_xscale=1
bulpon[i].image_yscale=1
bulpon[i].image_angle=20
bulpon_nagraj_x[i]=x+400
bulpon_nagraj_y[i]=y+50