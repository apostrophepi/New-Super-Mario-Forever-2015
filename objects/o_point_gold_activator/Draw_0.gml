...
draw_sprite_ext(global.gfx_point_gold_activator,0,x+20,y+20+downed+bump,1,1,bump2+sin(promien/5)*5,c_white,1)
draw_set_blend_mode(bm_add)
draw_sprite_ext(global.gfx_point_gold_activator,0,x+20,y+20+downed+bump,1+abs(bump/40),1+abs(bump/40),bump2+sin(promien/5)*5,c_white,abs(bump/40))
...
if destroy=1
    ...
    if on=0 {on=1;lolo=instance_create(x,y,o_literkowania);lolo.wartosc="Gold_Switch_ON";with(o_point_gold_block){on=1};with(o_point_gold_block_offed){on=1};break}
    if on=1 {on=0;lolo=instance_create(x,y,o_literkowania);lolo.wartosc="Gold_Switch_OFF";with(o_point_gold_block){on=1};with(o_point_gold_block_offed){on=1};break}}
    if global.english_or_polish=1 {
    if on=0 {on=1;lolo=instance_create(x,y,o_literkowania);lolo.wartosc="Złoty_włącznik_włączony";with(o_point_gold_block){on=1};with(o_point_gold_block_offed){on=1};break}
    if on=1 {on=0;lolo=instance_create(x,y,o_literkowania);lolo.wartosc="Złoty_włącznik_wyłączony";with(o_point_gold_block){on=1};with(o_point_gold_block_offed){on=1};break}}
    }
}
