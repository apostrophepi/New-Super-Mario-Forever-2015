if global.pauza=0{
    {
    if place_meeting(x,y+30,o_lava_define)
        {
        if hotted_by_lawa<1 {hotted_by_lawa+=0.01}
        lawed=10
        }
    if !place_meeting(x,y+30,o_lava_define)
        {
        if hotted_by_lawa>0 {hotted_by_lawa-=0.01}
        if lawed>0 {
        lawed-=1
        if random(100)>10 {repeat(2){part_particles_create(global.particle_sys_przed_morkami,x+random(140)-random(140),y+random(10)-random(10),global.part_effect_water_sparkle_lawa2,1)}}}
        
        }
    // WINDA AKTYWOWANA - PRZESOWANA JAK MARIO WSKOCZY
        if rodzaj=301 {
        if place_meeting(x,y,o_lift_blocker_odbijacz) {chmurkuj=1}
        if kat<180 && chmurkuj=1 {kat+=1}
        if kat<90 && chmurkuj=0 {kat+=1}
        if place_meeting(x,y-2,obj_mario) && obj_mario.sekwencja=0 && aktywuj=0
            {
            aktywuj=1
            kat=0 
            szybkosc=1
            jak_daleko_jedzie=wielkosc_kolka // dla windy obracanej RODZAJ=1
            }
