...
if global.pauza=0
    ...
    // skacze na mario
    if obj_mario.x>x-100 && obj_mario.x<x+100 && skacze_na_mario=0 && wyskok_sekwencja=0 {SXMS_SFX_PlayExt(405,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);skacze_na_mario=1;wyskok=-15;wyskok_sekwencja=1}
    if wyskok_sekwencja=1 {x+=4;y+=wyskok;wyskok+=0.1;if wyskok>=-5 {wyskok=-5}; powiekszator+=0.01
    if y<=__view_get( e__VW.YView, 0 )-300 {SXMS_SFX_PlayExt(406,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);instance_destroy();instance_create(x+350,y,o_WIELKI_ZJADACZOR_LECI)}
