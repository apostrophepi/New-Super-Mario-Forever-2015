wylacz+=1
if wylacz<5 {obj_mario_anim.animacja=6}


sampluj1+=1
if sampluj1=200 && global.english_or_polish=0 {global.bufor=SXMS_SFX_PlayExt(402,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);global.sciszacz_sekwencyjny_muzyki=5}
if sampluj1=200 && global.english_or_polish=1 {global.bufor=SXMS_SFX_PlayExt(400,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);global.sciszacz_sekwencyjny_muzyki=5}

if sampluj2>=1 {sampluj2+=1}
if sampluj2=60 && global.english_or_polish=0 {global.bufor=SXMS_SFX_PlayExt(403,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);global.sciszacz_sekwencyjny_muzyki=5}
if sampluj2=60 && global.english_or_polish=1 {global.bufor=SXMS_SFX_PlayExt(401,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);global.sciszacz_sekwencyjny_muzyki=5}
...
if animsek<>2 {
//if hello=0 {SXMS_SFX_PlayExt(round(random(1))+119,((x-view_xview[0])/800)*255,global.glosnosc_dzwieku,-1,0,0);hello=1}

draw_sprite_ext(s_toad_talker_4,0,x+20,y+ygrek,1,1,sin(degtorad(stream/5))/2,c_white,1)
draw_sprite_ext(s_toad_talker_5,0,x+20-11,y-5+ygrek,1,1,sin(degtorad(-stream/5+180+30))*50,c_white,1)
...

{
    ...
    if animsek=2
        ...
        if sekwencor<100 {sekwencor+=1;kierunek=-1}
        if sekwencja=90 {SXMS_SFX_PlayExt(round(random(1))+119,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0)}
        if sekwencor=100
            {
            kierunek=-1
            animsek=2
            max_x=10
            szybkosc=4
            if wygenerowal=1 {sekwencor=101;animsek=0}
            }
...
if kefir<>SXMS_SFX_Position(1,global.bufor,0) {odliczator=0;kefir=SXMS_SFX_Position(1,global.bufor,0)}
        ...
        if sekwencor=101 && odliczator>30
            {
            animsek=0
            if !instance_exists(o_dialog2) {instance_create(x,y,o_peach_talker_1);sekwencor=102}
            }
        if sekwencor=102
            {
            animsek=0
        ...
        if sekwencor=103
            ...
            szybkosc=3
            wygenerowal=0
            obj_mario.scrool_animation_fun=obj_mario.scrool_animation_fun/1.06
            }
        
if kierunek=1 && !place_meeting(x+2+szybkosc,y-abs(szybkosc)-3,obj_wall) && wygenerowal=0 {x+=szybkosc;max_x+=szybkosc}
if kierunek=-1 && !place_meeting(x-2+szybkosc,y-abs(szybkosc)-3,obj_wall) && wygenerowal=0  {x-=szybkosc;max_x-=szybkosc}
...
if generuj_dialog=1 && sekwencor<103
    {
    if x>obj_mario.x-100 && x<obj_mario.x+100 && y>obj_mario.y-100 && y<obj_mario.y+100 && wygenerowal=0
        {
...
if generuj_dialog=3 && x>obj_mario.x-100 && x<obj_mario.x+100 && y>obj_mario.y-100 && y<obj_mario.y+100 && klikers=0
    {
    klikers=1
    generuj_dialog=1
    }
