if global.pauza=0{
    ...
    //samlopwanie
    dodawaj_sampel+=1
    if sampluj_machine=2 && dodawaj_sampel>60*5{dodawaj_sampel=0
kanal=SXMS_SFX_PlayExt(318,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0)


}
if sampluj_machine=2 && dodawaj_sampel>0 {
    SXMS_SFX_Pan(kanal,((x-__view_get( e__VW.XView, 0 ))/800)*255)}
    
    
    
    // sterowniki
    if place_meeting(x,y,o_odbij_zamkowo_do_gory)
        {
        rodzaj=4
        kat=180
        }
    if place_meeting(x,y,o_odbij_zamkowo_na_dol)
        {
        rodzaj=4
        kat=0
        }     
    if place_meeting(x,y,o_odbij_zamkowo_w_lewo)
        {
        rodzaj=3
        kat=-90
        }          
    if place_meeting(x,y,o_odbij_zamkowo_w_prawo)
        {
        rodzaj=3
        kat=90
        }           
