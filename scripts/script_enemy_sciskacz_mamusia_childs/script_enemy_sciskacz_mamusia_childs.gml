function script_enemy_sciskacz_mamusia_childs() {
	if global.pauza=0{
	...
	if grawitacja>180 {grawitacja=0;sekwencja=1;ruch_combo=30+round(random(1))}
	if sekwencja=1
	    {
	    wielkosc_x+=0.015
	    if wielkosc_x>=0.40 {sekwencja=2}
	    }
	if sekwencja=2 {if instance_exists(nalezy_do) {direction=point_direction(x,y,nalezy_do.x,nalezy_do.y)+random(10)-random(10)}; speed=8+image_index*5; sekwencja=3}
	if sekwencja=3
	    {
	    if wielkosc_x<=0.10 {if ruch_combo<=0{sekwencja=0}; if ruch_combo>0 {sekwencja=1;ruch_combo-=1}}
	    }
    
	if !place_meeting(x,y-30,o_water_define) {motion_add(270,3)};
	if !place_meeting(x,y+30,o_water_define) {motion_add(90,3)};
	if !place_meeting(x+30,y,o_water_define) {motion_add(180,3)};
	if !place_meeting(x-30,y,o_water_define) {motion_add(0,3)}; 
	speed=speed/1.1
	if sekwencja<>1 {wielkosc_x=wielkosc_x/(1.13+image_index/20)}
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.2
	    {
	    draw_sprite_ext(global.gfx_enemy_sciskacz,0,x,y,0.30+wielkosc_x/3-sin(stream/2)/30,0.30-wielkosc_x/3+sin(stream/2)/30,-hspeed*5-vspeed*5,c_white,1)
	    if mrugacz2>0
	        {
	        draw_sprite_ext(global.gfx_enemy_sciskacz_2,0,x,y,0.30+wielkosc_x/3-sin(stream/2)/30,0.30-wielkosc_x/3+sin(stream/2)/30,-hspeed*5-vspeed*5,c_white,mrugacz2)
	        }
	    }
