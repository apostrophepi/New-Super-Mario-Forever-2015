function script_enemy_Cheep_Chomp_sterowany() {
	...
	if wyskok_sekwencja=0
	    {
	    wyskok_timer+=1
	    if wyskok_timer>300 {wyskok_sekwencja=1;wyskok_timer=0}
	    }
	if wyskok_sekwencja=1
	    {
	    if x>obj_mario.x+100 && x<obj_mario.x+250 && kierunek=-1 {wyskok_sekwencja=2; iyor=-10}
	    if x<obj_mario.x-100 && x>obj_mario.x-250 && kierunek=1 {wyskok_sekwencja=2; iyor=-10}
	    }
	...
	if x>obj_mario.x && place_meeting(x-180,y,o_water_define) {speed-=0.1}
	if x<obj_mario.x && place_meeting(x+180,y,o_water_define) {speed+=0.1}
	...
	if !place_meeting(x+120,y+120,o_water_define) && wyskok_sekwencja<>0 {speed=0;x=xprevious};
	if !place_meeting(x-120,y+120,o_water_define) && wyskok_sekwencja<>0 {speed=0;x=xprevious};
	...
	if kill=10
	    ...
	    instance_destroy()
	    fifi=instance_create(x,y,o_cheep_chomb_waiter)
	    fifi.ixor=xstart
	    fifi.iyor=ystart
	    }
	if kill=1000
