function script_enemy_rex() {
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)>global.odleglosc_ekranu*1.5 {seened=0}
	    ...
	    {
	    x+=kierunek*2
	    if place_meeting(x,y,obj_wall) {y-=2}
	    if !place_meeting(x,y+1,obj_wall) && !collision_rectangle(x,y+64,x+32,y+65,o_subplatform,0,1) {y+=1}
	    }

	if place_meeting(x+kierunek*2,y-2,obj_wall)
	    {
	    kierunek=kierunek*-1
	    }
	...
	if !place_meeting(x,y+1,obj_wall) && !collision_rectangle(x,y+64,x+32,y+65,o_subplatform,0,1) {sekwencja=1}
	if sekwencja=1
	    {
	    if global.is_subplatforms=1
	        ...
	        for (i=0;i<ceil(grawitacja);i+=1)
	            {
	            if collision_rectangle(x,y+64+i,x+32,y+65+i,o_subplatform,0,1) {y+=i;sekwencja=0; grawitacja=0;break;break;break}
	            }
	        }
	...
	if kill=1
	    ...
	    lolo=instance_create(x,y,o_enemy_rex_mini);
	    lolo.grafika=o_enemy_rex
	...
	draw_sprite_ext(s_rex_3,0,x+5,y+50+sin(stream),kierunek,1,sin(stream)*40+360,c_white,1)
	draw_sprite_ext(s_rex_1,0,x+5,y+30+sin(stream)*2,kierunek,1,sin(stream)*5+360,c_white,1)
	draw_sprite_ext(s_rex_3,0,x+5,y+50+sin(stream),kierunek,1,sin(-stream)*40+360,c_white,1)
	draw_sprite_ext(s_rex_2,0,x+5,y+40+sin(stream),kierunek,1,sin(stream)*40+360,c_white,1)}
