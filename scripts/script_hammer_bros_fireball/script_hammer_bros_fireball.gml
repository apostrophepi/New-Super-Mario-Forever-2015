function script_hammer_bros_fireball() {
	if global.pauza=0{
	...
	if kill=1
	    ...
	    lolo.grafika=global.gfx_troopa_7
	...
	draw_sprite_ext(global.gfx_troopa_6,0,x+16,y+sin(stream)*2-10+25,kierunek,1,sin(stream)*8+360,c_white,1)
	draw_sprite_ext(global.gfx_troopa_leg,0,x+18,y+20+sin(stream)+25,kierunek,1,(sin(-stream)*40)/jump_anim+360-jump_anim*kierunek,c_white,1)

	if atakowanie>=100 {draw_sprite_ext(s_fireball_boom,0,
	x+18+sin(ogniowanie-45*kierunek+90*kierunek)*20,
	y+45+cos(ogniowanie-45*kierunek+90*kierunek)*20,
	abs(sin(degtorad(stream*40)))/1.5+0.1,abs(sin(degtorad(stream*40)))/1.5+0.1,0,c_white,(atakowanie-100)/100)}

	draw_sprite_ext(global.gfx_troopa_arm,0,x+18,y+5+sin(stream)+25,kierunek+rzut_anim/80*kierunek,1+rzut_anim/80,sin(stream)*40+360+rzut_anim*kierunek,c_white,1)
	}
