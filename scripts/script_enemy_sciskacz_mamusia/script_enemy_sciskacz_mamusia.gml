function script_enemy_sciskacz_mamusia() {
	if global.pauza=0{
	...
	if grawitacja>180 {grawitacja=0;sekwencja=1;ruch_combo=round(random(1))}
	if sekwencja=1
	    {
	    wielkosc_x+=0.015
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.2
	    {
	    draw_sprite_ext(global.gfx_enemy_sciskacz,0,x,y,0.50+wielkosc_x/2-sin(stream/2)/30,0.50-wielkosc_x/2+sin(stream/2)/30,-hspeed*5-vspeed*5,c_white,1)
	    if mrugacz2>0
	        {
	        draw_sprite_ext(global.gfx_enemy_sciskacz_2,0,x,y,0.50+wielkosc_x/2-sin(stream/2)/30,0.50-wielkosc_x/2+sin(stream/2)/30,-hspeed*5-vspeed*5,c_white,mrugacz2)
	        }
	    }
