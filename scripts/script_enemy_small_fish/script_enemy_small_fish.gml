function script_enemy_small_fish() {
	if global.pauza=0{
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.2 {



	if kierunek=1 {speed=1; direction=sin(stream/4)*10}
	if kierunek=-1 {speed=1; direction=180-sin(stream/4)*10}
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.2
	    ...
	    draw_sprite_ext(global.gfx_enemy_big_fish[image_index],0,x,y,-kierunek-sin(stream/2)/50,1+sin(stream/2)/50,sin(stream/4)*10,c_white,1)
	    draw_sprite_ext(global.gfx_enemy_big_fish_2,0,x-5*kierunek,y,-kierunek/3-sin(stream/2)/50,1/3+sin(stream/2)/50,sin(stream/2)*40,c_white,1)
