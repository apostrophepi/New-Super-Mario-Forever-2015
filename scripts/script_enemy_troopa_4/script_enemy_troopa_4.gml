function script_enemy_troopa_4() {
	if global.pauza=0{
	...
	rzuca+=1
	if rzuca>400 {rzuca=0; rzucil=100;SXMS_SFX_PlayExt(45,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);lolo=instance_create(x+20,y+20,o_enemy_fireballs);lolo.ixor=4*kierunek;lolo.iyor=-2-random(3)}
	if rzucil>0 {rzucil=rzucil/1.3}
	...
	ogniowanie=sin(stream)/1.3
	draw_sprite_ext(global.gfx_troopa_leg,0,x+18,y+20+sin(stream),kierunek,1,sin(stream)*10+360+kierunek*-20,c_white,1)
	draw_sprite_ext(global.gfx_troopa_2,0,x+16,y+sin(stream)*2,kierunek,1,sin(stream)*5+360,c_white,1)
	draw_sprite_ext(global.gfx_troopa_leg,0,x+18,y+20+sin(stream),kierunek,1,sin(-stream)*10+360+kierunek*-20,c_white,1)
	if rzuca>=200 {draw_sprite_ext(s_fireball_boom,0,x+18+sin(ogniowanie-45*kierunek+90*kierunek)*15,y+5+cos(ogniowanie-45*kierunek+90*kierunek)*15,abs(sin(degtorad(stream*40)))/1.5+0.1,abs(sin(degtorad(stream*40)))/1.5+0.1,0,c_white,(rzuca-200)/200)}
	draw_sprite_ext(global.gfx_troopa_arm,0,x+18,y+5+sin(stream),kierunek+rzucil/400,1+rzucil/400,sin(stream)*40+360-rzucil*kierunek,c_white,1)
