function script_paralax_step2_darker() {
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*visible_size
	    ...
	    draw_sprite_ext(sprite_index,0,x,y,image_xscale+sin(stream)/8,image_yscale-sin(stream)/8,sin(stream)*2,c_white,image_alpha+abs(sin(stream))/2)
