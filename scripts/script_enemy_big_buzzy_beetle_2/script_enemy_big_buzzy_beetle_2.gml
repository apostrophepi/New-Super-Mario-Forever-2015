function script_enemy_big_buzzy_beetle_2() {
	if global.pauza=0{
	...
	if kill=1000
	    ...
	    SXMS_SFX_PlayExt(51,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0)
	    lolo=instance_create(x,y,o_enemy_kill);
	    lolo.grafika=s_big_buzzy_beetle_3
	    lolo.kierunek_idzie=kierunek
	    instance_destroy()
	    }
	...
	if rodzaj=0 {draw_sprite_ext(s_big_buzzy_beetle_3,0,x+35,y+15+sin(stream)*2,kierunek,1,sin(stream)*5+360,c_white,1)}
	if rodzaj=1 {draw_sprite_ext(s_big_buzzy_beetle_3,0,x+35,y+15+sin(stream)*2,kierunek,1,sin(stream)*15+360,c_white,1)}
	if rodzaj=2 {draw_sprite_ext(s_big_buzzy_beetle_3,0,x+35,y+15+sin(stream)*2,kierunek,1,sin(stream)*15+360,c_white,1)}
	}
