function sript_enemy_bujany_wiatrak() {
	if global.pauza=0
	{
	    {   
	    x=start_x
	    ...
	    kolec.x=start_x+sin(degtorad(kat+2*dlugosc*sin(degtorad(kat))))*(odleglosc*dlugosc)
	    kolec.y=start_y+cos(degtorad(kat+2*dlugosc*sin(degtorad(kat))))*(odleglosc*dlugosc)
	    kolec.image_angle=10*dlugosc*sin(degtorad(kat))
        
	    kat=kat_poczatkowy+sin(degtorad(szybkosc_dodana))*promien_bujania
	    szybkosc_dodana+=szybkosc
	    if szybkosc_dodana>360 {szybkosc_dodana=0}
    
	    dzwiek_bujania+=1
	        if point_distance(kolec.x,kolec.y,obj_mario.x,obj_mario.y)<220
	        {
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.6+dlugosc*20 {
	...
	for(i=0;i<dlugosc;i+=1)
	        ...
	        1,1,kat+90+4*i*sin(degtorad(kat)),c_white,1)
	        }
