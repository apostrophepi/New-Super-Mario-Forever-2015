function script_enemy_big_cannon2() {
	if global.pauza=0 {
	...
	if reload>=reload_shot
	    ...
	    reload=0
	    koko=instance_create(x,y,o_enemy_bullet_ball_3plus)
	    koko.direction=kierunek
	    if kierunek=180 {koko.obroc=2}
	    koko.speed=10
	    koko.depth=depth+10
	    koko.nalezy_do=id
	    koko.rodzaj=rodzaj
	...
	if (kierunek<90) || (kierunek>270) {draw_sprite_ext(s_big_cannon,0,x,y,1,1,kierunek,c_white,1)}
	if kierunek<=270 && kierunek>=90 {draw_sprite_ext(s_big_cannon,0,x,y,1,-1,kierunek,c_white,1)}
	if chwytak=1 {draw_sprite_ext(s_big_cannon_dol,0,x,y+20,1,1,chwytak_kat,c_white,1)}
	}
