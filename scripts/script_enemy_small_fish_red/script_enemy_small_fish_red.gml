function script_enemy_small_fish_red() {
	if global.pauza=0{
	...
	if kierunek=1 {speed=2; direction=sin(degtorad(stream))*30}
	if kierunek=-1 {speed=2; direction=180-sin(90+degtorad(stream+90))*30}
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.2
	    {
	    draw_sprite_ext(s_big_boo_marker,0,x,y,-kierunek/4,1/4,sin(degtorad(stream)/4)*10,c_white,0.5)
    
	    draw_sprite_ext(global.gfx_enemy_big_fish_4,0,x,y,-kierunek-sin(degtorad(stream))/50,1+sin(degtorad(stream))/50,sin(degtorad(stream))*30,c_white,1)
	    draw_sprite_ext(global.gfx_enemy_big_fish_2,0,x-5*kierunek,y,-kierunek/3-sin(degtorad(stream))/50,1/3+sin(degtorad(stream))/50,sin(degtorad(stream)/2)*40,c_white,1)
