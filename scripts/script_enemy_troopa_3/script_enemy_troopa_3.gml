function script_enemy_troopa_3() {
	if global.pauza=0{
	...
	rzuca+=1
	if rzuca>400 {rzuca=0; rzucil=100;SXMS_SFX_PlayExt(46,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);lolo=instance_create(x+20,y+20,o_enemy_hammers);lolo.ixor=4*kierunek}
	...
	draw_sprite_ext(global.gfx_troopa_leg,0,x+18,y+20+sin(stream),kierunek,1,sin(-stream)*10+360+kierunek*-20,c_white,1)
	if rzuca<200 {draw_sprite_ext(global.gfx_troopa_arm,0,x+18,y+5+sin(stream),kierunek+rzucil/400,1+rzucil/400,sin(stream)*40+360-rzucil*kierunek,c_white,min(1,(200-rzuca)/100))}
	if rzuca>=100 {draw_sprite_ext(global.gfx_troopa_arm_hammer,0,x+18,y+5+sin(stream),kierunek,1,sin(stream)*40+360,c_white,(rzuca-100)/100)}
