function script_enemy_dry_bones_samestroujacy() {
	if global.pauza=0{
	...
	if kill=1
	    ...
	    lolo=instance_create(x,y,o_enemy_dry_bones_clash_2);
	    instance_destroy()
	    }
	...
	draw_sprite_ext(s_red_glow,0,x+18,y+20+sin(stream),2-sin(stream)/2,2+sin(stream)/2,stream*60,c_white,0.5)
	draw_sprite_ext(s_red_glow,0,x+18,y+20+sin(stream),1+sin(stream)/10,1-sin(stream)/10,stream*60+180,c_white,1)
