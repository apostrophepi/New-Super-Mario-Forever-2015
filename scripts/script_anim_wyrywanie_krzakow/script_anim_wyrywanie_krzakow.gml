function script_anim_wyrywanie_krzakow() {
	// wyrywanie murkow i krzakow
	if animacja=9{
	niesie=1
	...
	if schylak_sek=0 
	    ...
	    if schylak>-40 && size2=-1 {schylak-=8}
	    if schylak<40 && size2=1 {schylak+=8}
	    run_promien=0
	    if schylak<=-40 || schylak>=40 {schylak_sek=1}
	    }
	if schylak_sek=1
	    {
	    if size2=-1 && schylak>-200 {schylak-=8}
	    if size2=1 && schylak<200 {schylak+=8}
	    schylanie=schylanie/1.13
	    run_pochylenie=run_pochylenie/1.16
