function script_enemy_troopa_5() {
	if global.pauza=0{
	...
	if sekwencja=1
	    {
	    if grawitacja<12 {grawitacja+=0.25};
	    if grawitacja<0 && !place_meeting(x,y-12,obj_wall) {y+=grawitacja};
	    if grawitacja<0 && place_meeting(x,y-12,obj_wall) {grawitacja=0};
	    if grawitacja>0 {y+=grawitacja};
    
	    if global.is_subplatforms=1 && grawitacja>0
	        {
