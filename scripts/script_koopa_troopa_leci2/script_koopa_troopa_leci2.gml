function script_koopa_troopa_leci2() {
	if global.pauza=0{
	...
	if kill=10
	    ...
	    if jaka_grafika=0 {lolo.grafika=s_big_buzzy_beetle}
	    if jaka_grafika=1 {lolo.grafika=s_big_buzzy_beetle}
	    if jaka_grafika=2 {lolo.grafika=s_big_buzzy_beetle}
	...
	if kill=1
	    ...
	    SXMS_SFX_PlayExt(51,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0)
	    lolo=instance_create(x,y,o_enemy_troopa_lezy2);
	...
	if point_distance(x,y,global.ekran_srodek_x,global.ekran_srodek_y)<=global.odleglosc_ekranu*1.1{
	if jaka_grafika=0 {draw_sprite_ext(s_big_buzzy_beetle,0,x+32,y+30+sin(stream)*2,1,1,360+sin(degtorad(globalny_kat))*10,c_white,1)}}
