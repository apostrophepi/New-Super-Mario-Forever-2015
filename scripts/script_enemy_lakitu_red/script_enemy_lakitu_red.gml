function script_enemy_lakitu_red() {
	if global.pauza=0
	    {
	    if energy<=0 {kill=10}
	    if aktywna=0
	    ...
	    if aktywna=1 && obj_mario.x>=room_width-2000
	            {
	            x+=ixor
	            if x>room_width-2000 && ixor>-10 {ixor-=0.5}
	            }
            
            
            
	    if aktywna=1 && obj_mario.x>600 && obj_mario.x<room_width-2000
	        {
	        ...
	        if lata_nadol=2
	            {
	            if y<obj_mario.y-150 && iyor<10 {iyor+=1}
	            if y>obj_mario.y+200 && iyor>-10 {iyor-=1}
	        ...
	        if sekwencja=2 && timer>200 
	            ...
	            if czym_rzuca=0 {lolo=instance_create(x,y,o_lakitu_throw);lolo.rodzaj=czym_rzuca;sekwencja=3;SXMS_SFX_PlayExt(77+round(random(2)),((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0)}
	...
	draw_sprite_ext(global.gfx_lakitus_1,0,x,y,kierunek-sin(degtorad(anim*4))/20,1+sin(degtorad(anim*4))/20,-ixor*2+360,c_red,1)
