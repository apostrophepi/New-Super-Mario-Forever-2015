function script_hammer_bros_boomerang() {
	if global.pauza=0{
	...
	if !instance_exists(jego_boomerang) {if atakowanie<80{atakowanie=80};atakowanie+=1.2}
	...
	if atakowanie>400
	    ...
	    SXMS_SFX_PlayExt(82,((x-__view_get( e__VW.XView, 0 ))/800)*255,global.glosnosc_dzwieku,-1,0,0);
	    lolo=instance_create(x+20,y+20,o_enemy_boomerang);
	    lolo.speed=5;
	    if kierunek=-1 {lolo.direction=180+random(20)-random(20)}; 
	    if kierunek=1 {lolo.direction=0+random(20)-random(20)};
	    lolo.nalezy_do=id
	    jego_boomerang=lolo.id
	    }
	...
	if kill=1
	    ...
	    lolo.grafika=global.gfx_troopa_8
	...
	draw_sprite_ext(global.gfx_troopa_5,0,x+16,y+sin(stream)*2-10+25,kierunek,1,sin(stream)*8+360+rzut_anim*kierunek/10,c_white,1)
	draw_sprite_ext(global.gfx_troopa_leg,0,x+18,y+20+sin(stream)+25,kierunek,1,(sin(-stream)*40)/jump_anim+360-jump_anim*kierunek,c_white,1)
	if atakowanie<120 {draw_sprite_ext(global.gfx_troopa_arm,0,x+18,y+5+sin(stream)+25,kierunek+rzut_anim/80*kierunek,1+rzut_anim/80,sin(stream)*40/(atakowanie/5)+360+rzut_anim*kierunek+(atakowanie*kierunek/3),c_white,1)}
	if atakowanie>100 {draw_sprite_ext(global.gfx_troopa_arm_boomerang,0,x+18,y+5+sin(stream)+25,kierunek,1,sin(stream)*40/(atakowanie/5)+360+rzut_anim*kierunek+(atakowanie*kierunek/3),c_white,(atakowanie-100)/20)}
